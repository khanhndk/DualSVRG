...
def log_result(result):
    ...
    print('Mistake rate: {0:.2f}%\nTraining time: {1} seconds. Par: {2}'.format(mistake_rate * 100, int(time), params))
...
def run_grid_search_multicore():
    ...
    # params = {'regular_param': 2.0 ** np.arange(-4, 16, 2) / 64700,
    #           'gamma': 2.0 ** np.arange(-8, 8, 2)}
    params = {'regular_param': [0.000061823802163833],
              'gamma': [0.00390625]}
    candidate_params_lst = list(ParameterGrid(params))

    pool = mp.Pool(processes=1)  # maximum of workers
    for candidate_params in candidate_params_lst:

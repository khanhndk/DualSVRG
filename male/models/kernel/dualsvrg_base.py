...
class BaseDualSVRG(Model):
    ...
    def __init__(self,
                 ...
                 show_loss=True,
                 **kwargs):
        super(BaseDualSVRG, self).__init__(model_name=model_name, **kwargs)
        self.mode = mode
    ...
    def _init(self):
        super(BaseDualSVRG, self)._init()
        np.seterr(under='ignore')

        learning_rate_m = 1 if self.loss_func == 'logistic' else 1.0 / self.smooth_hinge_tau
    ...
    def get_params(self, deep=True):
        out = super(BaseDualSVRG, self).get_params(deep=deep)
        param_names = BaseDualSVRG._get_param_names()

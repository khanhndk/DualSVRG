...
class OnlineDualSVRG(Model):
    ...
    def _get_grad_full(self, xn, yn):
        yn_pred, wxn, xn_rf, wxn_rf, dist2_xn, kn, wxn_core = self._predict_one_given_w(
            self.w_full_rf, self.w_full_core, xn
        )
    ...
    def _predict_one(self, xn):
        return self._predict_one_given_w(self.w_cur_rf, self.w_cur_core, xn)

    def _predict_one_given_w(self, w_rf, w_core, xn):
        xn_rf = self._calc_rf(xn)
        wxn_rf = np.sum(w_rf * xn_rf, axis=1)

        idx = np.sum(np.abs(w_core), axis=0) != 0
        dist2_xn = self._get_dist2(xn, self.idx_core[idx])
    ...
    def _fit_loop(self, x, y,
        ...
        for n in range(max_loop):
            progbar.header = 'Mistake rate: {0:.2f}% '.format(self.mistake_rate * 100)
            progbar.update(n)
            epoch_logs = {}
            ...
            grad_full, idx_full_runner, loss_full = self._get_grad_full(xnt, ynt)
            ...
            if move_decision[n]:
                ...
                idx_find = np.where(self.idx_core[:self.num_core] == nt)[0]
                if len(idx_find) > 1:
                    raise Exception()
                if len(idx_find) == 0:
                    self.num_core += 1
            ...
            if self.oracle == 'budget':
                ...
                num_remove = np.sum(w_value_tmp != 0) - self.core_max + 1
                ...
                if num_remove > 0:
                    w_value_tmp[w_value_tmp == 0] = np.nan
            ...
            if (n+1) % self.freq_update_full_model == 0:
                ...
                for i in range(n-self.cache_size+1, n+1):
                    ...
                    xit_tmp = x[it, :]
                    yit_tmp = y[it]
                    grad_full, idx_full_runner, loss_full = self._get_grad_full(xit_tmp, yit_tmp)
                    # if move_decision[i]:
                    if self.chk_core[it] < 0:
                        vit_rf_full = np.zeros((self.num_classes, self.rf_2dim_pad))
                        vit_rf_full[ynt, :] = grad_full * xnt_rf
                        vit_rf_full[idx_full_runner, :] = -grad_full * xnt_rf
        ...
        for n in range(max_loop):
            ...
            if len(grad_lst) > self.cache_size - 1:
                ...
                if self.oracle == 'budget':
                    ...
                    # if move_decision[n - self.cache_size]:
                    if self.chk_core[idx_pop] < 0:
                        sum_grad_rf -= grad_tmp
                    else:
                        sum_grad_core[:, self.chk_core[idx_pop]] -= grad_tmp
            xnt_rf_lst.append(xnt)
